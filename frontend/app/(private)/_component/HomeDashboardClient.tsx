...
export default function HomeDashboardClient({ stats, universities, deadlines }: Props) {
  ...
  const topCountries = useMemo(() => countries.slice(0, 6), [countries]);
  const topCourses = useMemo(() => courses.slice(0, 6), [courses]);

  if (showSplash) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[linear-gradient(120deg,#4A90E2_0%,#357ABD_100%)]">
        <div className="text-center text-white">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-white/80">UniGuide</p>
          <h2 className="mt-2 text-4xl font-bold">AI University Finder</h2>
          <p className="mt-3 text-sm text-white/90">Loading your personalized dashboard...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        {filteredUniversities.slice(0, 3).map((uni) => (
          <article
            key={`hero-uni-${uni.id}`}
            className="rounded-[8px] border border-[#d8e5f8] bg-white p-5 shadow-[0_4px_8px_rgba(0,0,0,0.08)]"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-lg font-bold text-[#333333]">{uni.name}</p>
            ...
            <p className="mt-3 text-sm text-[#666666]">
              {uni.country} • {uni.city} • {uni.score} fit
            </p>
            <button
              type="button"
              onClick={() => setSelectedUniversity(uni)}
              className="mt-4 rounded-[8px] bg-[#4A90E2] px-4 py-2 text-xs font-bold uppercase tracking-[0.08em] text-white hover:bg-[#357ABD]"
            >
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="rounded-[8px] border border-[#d8e5f8] bg-white p-5 shadow-[0_4px_8px_rgba(0,0,0,0.08)]">
          <h3 className="text-lg font-bold text-[#333333]">Country Cards</h3>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {topCountries.map((country) => {
              const countrySample = universities.find((u) => u.country === country.name);
              return (
                <button
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold text-[#333333]">{country.name}</p>
                    <p className="text-xs text-[#666666]">{country.count} universities</p>
                  </div>
                </button>
              );
            })}
        ...
        <article className="rounded-[8px] border border-[#d8e5f8] bg-white p-5 shadow-[0_4px_8px_rgba(0,0,0,0.08)]">
          <h3 className="text-lg font-bold text-[#333333]">Courses Cards</h3>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {topCourses.map((course) => (
              <button
                ...
                className="rounded-[8px] border border-[#d8e5f8] p-3 text-left hover:bg-[#f5f9ff]"
              >
                <p className="text-sm font-semibold text-[#333333]">{course.name}</p>
                <p className="text-xs text-[#666666]">{course.count} universities offer this</p>
              </button>
            ))}
          </div>
        </article>
      ...
      <AIChatbot universities={universities} />
    </div>
  );
}
